...
typedef struct _GETCDHAND {
  ...
  unsigned char jitterCorr;       /* use built-in jitter correction?     */
...
unsigned char QueryCDParms( HCDROM hCD, int which, DWORD *pNum );
unsigned char ModifyCDParms( HCDROM hCD, int which, DWORD val );
HCDROM GetCDHandle( LPGETCDHAND lpcd );
unsigned char CloseCDHandle( HCDROM );
...
DWORD CDDBGetOptionInt( int what, unsigned char *bValid );
char *CDDBGetOptionText( int what, char *szBuf, int maxLen, unsigned char *bValid );
DWORD CDDBGetServerList( LPCDDBSITELIST lpSiteList );
DWORD CDDBSubmit( DWORD dwDiscID, unsigned char bTest, char *szEmail, char *szCategory,
		  char *szEntry );
...
DWORD startStopUnit( HCDROM hCD, unsigned char bLoEj, unsigned char bStart );
DWORD pauseResumeCD( HCDROM hCD, unsigned char bPause );
